...
@QueryHOC(query)
@withStyles(styles)
export default class CategoryLocationsUpdater extends Component {
  ...
  handleDelete(locationId) {
    const variables = { locationId, categoryId: this.props.categoryId };
    this.props.mutateRemoveLocation({ variables })
      .then(this.props.refetchCategory)
      .then(this.forceUpdate());
  }

  handleChange(e) {
    if (e.target.value === 'none') return;
    const variables = { locationId: e.target.value, categoryId: this.props.categoryId };
